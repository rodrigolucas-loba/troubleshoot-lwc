...
function translateIssueText(value) {
  return String(value || '')
    ...
    .replaceAll('The annotation @isTest should be in PascalCase: @IsTest', 'A anotacao @isTest deve usar PascalCase: @IsTest')
    .replaceAll('Apex unit test classes should have at least one System.runAs() call', 'As classes de teste Apex devem incluir pelo menos uma chamada a System.runAs()')
    .replaceAll('Avoid SOQL queries without a where or limit statement', 'Evitar queries SOQL sem clausula where ou limit')
    .replaceAll('Validate CRUD permission before SOQL/DML operation or enforce user mode', 'Validar permissao CRUD antes de operacoes SOQL ou DML, ou impor user mode')
    .trim();
}
...
function buildFallback(summary) {
  ...
  const recommendedAction = critical > 0
    ? 'Corrigir os findings criticos antes do merge ou da promocao.'
    : high > 0
      ? 'Priorizar os findings high e validar o impacto antes do merge.'
      : total > 0
        ? 'Rever os findings moderados e baixos antes de fechar a branch.'
...
function normalizeAiResult(candidate, fallback) {
  ...
  return {
    ...
    topIssues: Array.isArray(candidate.topIssues)
      ...
      : fallback.topIssues,
    source: String(candidate.source || 'gemini').trim() || 'gemini'
  };
}
  ...
  if (apiKey) {
    ...
    if (response?.ok) {
      try {
        ...
        finalResult = {
          ...fallback,
          summary: `${fallback.summary} O Gemini respondeu fora do formato esperado e foi usado fallback local.`,
          source: 'fallback'
        };
      }
    } else if (response) {
